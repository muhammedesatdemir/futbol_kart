...
Oyuncu ana sayfada bir **oyun modu** seçer; her mod aynı oyuncu havuzunu kullanır, kendi sahne akışına sahiptir ve **bota karşı** + **arkadaşa karşı (hot-seat)** oynanır. **VS Düello, Hedefe Yaklaş ve Kadro Kur ayrıca online** (gerçek rakiple, sunucu-otoriteli) oynanabilir — bkz. [🌐 Online Mod](#-online-mod--vs-düello-gerçek-zamanlı-multiplayer). Dört mod:
...
> **Durum (mod olgunluğu):** Dört mod da **içerik olarak canlıya hazır.** VS Düello 106 şablon; **Liste Doldur 235**, **Hedefe Yaklaş 205**, **Kadro Kur 151** kriter (toplam **697** — kriter ÜRETİCİSİ ile alan×filtre kombinasyonundan türetilir, `criteriaCatalog.ts`). Her oyun OTURUMU rastgele farklı kriter seçer (`roundSeed`). **VS Düello, Hedefe Yaklaş ve Kadro Kur online'ı çalışıyor** (sunucu-otoriteli + Ably). Geriye kalan: online'ı son moda (Liste Doldur) yayma + kulüp-bazlı yeni modlar (bkz. [VERI.md](VERI.md)).
...
#### 🌐 Online Mod — gerçek zamanlı multiplayer (VS Düello + Hedefe Yaklaş + Kadro Kur)
...
**Hedefe Yaklaş** ve **Kadro Kur** da artık online oynanabilir — aynı altyapının (eşleştirme, Ably, optimistic-lock, versiyon-GET, süre-dolumu) üzerine mod-özel sunucu motoru + client köprüsü eklenerek (aşağıda "Diğer modlara yayma").
...
> Tam plan, fazlar ve kararlar: [ONLINE-YOL-HARITASI.md](ONLINE-YOL-HARITASI.md) + [PLAN.md §19](PLAN.md). Kalan: **Liste Doldur** (aynı şablon — en zor: spoiler koruması + can sistemi).
...
- 🟡 **Online multiplayer** — **VS Düello + Hedefe Yaklaş + Kadro Kur için CANLI** (sunucu-otoriteli + Ably hibrit push; bkz. [🌐 Online Mod](#-online-mod--vs-düello-gerçek-zamanlı-multiplayer)). Kalan: online'ı son moda (Liste Doldur) yayma, rating/Elo hesabı (şema hazır), Google OAuth env (kod hazır), kopma/reconnect kenar durumları.
