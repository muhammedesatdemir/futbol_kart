...
> **DerbyGoal** — 8.912 futbolculuk veri üzerinde **10 oyun modu** sunan, hot-seat · bota karşı **ve online** oynanan dijital futbol kart & tahmin oyunu (9 mod offline+online; **İmposter** 3-5 kişilik online-özel sosyal dedüksiyon).
...
Oyuncu ana sayfada bir **oyun modu** seçer; her mod aynı oyuncu havuzunu kullanır, kendi sahne akışına sahiptir. İlk dokuz mod **bota karşı** + **arkadaşa karşı (hot-seat)** + **online** (gerçek rakiple, sunucu-otoriteli) oynanır; onuncu mod **İmposter** ise 3-5 kişilik **online-özel** sosyal dedüksiyondur — bkz. [🌐 Online Mod](#-online-mod--vs-düello-gerçek-zamanlı-multiplayer). On mod:
...
- **🕵️ İmposter** — *(SADECE online · 3-5 kişi · sosyal dedüksiyon)* Lobi (rastgele 30sn/3-5 esnek veya davet linki) dolunca herkes rolünü görür: biri gizli **imposter**, diğerleri **masum**. Masumlara gizli bir **kalburüstü futbolcunun adı** verilir; imposter'a verilmez — onun yerine her oyun farklı eksenden **TEK bulanık ipucu** (pozisyon/dönem/kupa/fiziksel/büyük-milliyet — ifşa korumalı). **3 tur sıra-tabanlı kelime** (herkes futbolcuyu ima eden kısa kelime yazar, anlık görünür; futbolcu/kulüp adı yasak), sonra **gizli oylama**. İmposter NET en çok oyu almazsa (berabere dahil) → **imposter kazanır**. Yeni `match_player` tablosu + N-kişi lobi matchmaking; rol/oy/cevap reveal'a kadar **sunucuda maskeli** (`viewImposterState`); imposter kimliği `crypto` rastgeleden (seed'den türetilemez). *v1: chat + bota-karşı yok (v2'ye).*

> **Durum (mod olgunluğu):** On mod da **içerik olarak canlıya hazır.** VS Düello 106 şablon; **Liste Doldur 235**, **Hedefe Yaklaş 205**, **Kadro Kur 151** kriter (kriter ÜRETİCİSİ ile alan×filtre kombinasyonundan türetilir, `criteriaCatalog.ts`); **4'lü Kıyas 570+** soru çeşidi (26 metrik × pozisyon × çağ/milliyet filtre, prune'lu üreteç); **Kareleri Kap** + **Zincir Kur** + **Ortak Bul** + **Kariyer Yolu** kulüp/kariyer-bazlı (kürasyonlu rastgele → her oyun benzersiz). **İmposter** 3-5 kişilik online sosyal dedüksiyon (yeni `match_player` tablosu + N-kişi lobi). Her oyun OTURUMU rastgele farklı kriter/matris/kulüp/çift/kariyer/gizli-futbolcu seçer (`roundSeed`; imposter `imposterPool.json` rank 1-585'ten %60/%40 bant). **On modun da online'ı çalışıyor** (sunucu-otoriteli + Ably; Liste/Ortak/Kariyer/4'lü Kıyas/İmposter'da cevap/rol sunucuda gizli = hile koruması). Geriye kalan: rating/Elo hesabı (şema hazır) + **İki Takım Ortak** (veri hazır `clubPairs.json`, ayrı mekanik) + **İmposter v2** (chat + bota-karşı) — bkz. [VERI.md](VERI.md) + [PLAN.md §14-23](PLAN.md).
...
| Metrik | Değer |
|---|---|
| **Oyun modu** | 10 (VS Düello · Kadro Kur · Hedefe Yaklaş · Liste Doldur · Kareleri Kap · Zincir Kur · Ortak Bul · Kariyer Yolu · 4'lü Kıyas · İmposter) — ilk 9'u bota+arkadaşa+**online**, İmposter 3-5 kişilik **online-özel** |
...
**Aktif geliştirme — 10 mod içerik olarak canlıya hazır; 9'u bota+arkadaşa+online, İmposter online-özel.** **VS Düello** tam olgun (106 şablon + 3 joker + 3 zorunlu kategori bonusu + geri sayım süreleri); **Kadro Kur (151 kriter)**, **Hedefe Yaklaş (205 kriter)**, **Liste Doldur (235 kriter)** modları zengin içerikle canlı — kriter ÜRETİCİSİ ([`criteriaCatalog.ts`](apps/web/src/lib/criteriaCatalog.ts)) ile alan×filtre (pozisyon/aktiflik/milliyet) kombinasyonundan türetilir; her oyun OTURUMU `roundSeed` ile farklı kriter seçer (sağlıksız kombinasyonlar `prune*`/`resolveTargetBands` ile elenir). **4'lü Kıyas (570+ soru çeşidi)** aynı üreteç felsefesiyle (26 metrik × pozisyon × çağ/milliyet filtre, prune'lu — ölü tur yok) + 2 joker (%50 / x2). **Kareleri Kap** (5×5 bitişik-kulüp matrisi) + **Zincir Kur** (7-kulüp keşişim) + **Ortak Bul** (2-kulüp ortak oyuncu, nadirlik puanı) + **Kariyer Yolu** (kademeli ipucu) ise **kulüp/kariyer-bazlı** kürasyonlu modlar — `clubPool.json`/`clubPairs.json` + `players[].clubs[]`'tan kürasyonlu rastgele üretilir, öneri/ipucu jokerleriyle. **İmposter** 3-5 kişilik online sosyal dedüksiyon: yeni `match_player` tablosu + N-kişi lobi matchmaking (atomik claim) + sıra-tabanlı kelime + gizli oylama; rol/cevap sunucuda maskeli, imposter kimliği `crypto` rastgeleden. **On modun da online multiplayer'ı çalışıyor** (sunucu-otoriteli motor + Ably hibrit push + Neon Postgres; bkz. [🌐 Online Mod](#-online-mod--vs-düello-gerçek-zamanlı-multiplayer)). Mod-agnostik altyapı (atomik matchmaking, optimistic-lock, versiyon-GET, süre-dolumu) + mod-özel sunucu motoru + client köprüsü deseniyle her mod online'a taşındı; Liste Doldur/Ortak Bul/Kariyer Yolu/4'lü Kıyas/İmposter'da **cevap/rol sunucuda gizli** (spoiler/hile koruması), Kareleri Kap/Zincir'de puanlama (bitişik grup BFS / keşişim) sunucuda hesaplanır. Veri katmanı doğrulandı, atmosfer cilası tamam. **Backend bağlı:** Neon Postgres (Frankfurt), migration'lar uygulandı (en son `0003` match_player), Better-Auth (e-posta/şifre + magic-link yedek, doğrulanmış domain; hata mesajları Türkçe) + Ably realtime aktif — online maçlar uçtan uca test edildi. **Arkadaşını davet et** (özel maç linki) tüm modlarda çalışıyor. **Canlı:** Vercel'de [`derbygoal.com`](https://derbygoal.com) (Frankfurt fonksiyon bölgesi). **Kalan:** rating/Elo (şema hazır) + İki Takım Ortak (veri hazır, ayrı mekanik) + İmposter v2 (chat + bota-karşı) — bkz. [VERI.md](VERI.md) + [PLAN.md §14-23](PLAN.md).

### Tamamlananlar

#### 🎮 Oyun modları (10 mod — ortak omurga)

İlk dokuz mod ana sayfadaki **oyun-modu seçimiyle** (`GameModeSelectScene`) açılır, kendi route'una sahiptir (`/oyna` · `/kadro` · `/hedefe-yaklas` · `/liste-doldur` · `/kareleri-kap` · `/zincir` · `/ortak-bul` · `/kariyer` · `/4lu-kiyas`), ve **paylaşılan omurgayı** kullanır: rakip seçimi (`OpponentSelectScene` — bota/arkadaşa karşı), faz-bilinçli geri navigasyon, sahne arka planları (`SceneBackground` bgKey override), geri sayım (`CountdownRing`), snake draft mantığı, sıralı reveal + konfeti + ses (`useSfx`/`Confetti`), isim modalı (`NameModal`). Onuncu mod **İmposter** (`/imposter`) online-özel, N-kişilik (`match_player`). Her modun **saf mantık katmanı** ayrıdır (test edilebilir, DOM'suz): `squadMode.ts` / `targetMode.ts` / `listMode.ts` / `squaresMode.ts` / `chainMode.ts` / `commonMode.ts` / `careerMode.ts` / `quizMode.ts` / `imposterMode.ts`.
...
- ✅ **İmposter** *(SADECE online, N-kişi, `imposterMode.ts`)* — 3-5 kişilik sosyal dedüksiyon (Among Us tarzı). Lobi (rastgele 30sn/3-5 esnek **veya** davet linki) → herkese kişiye-özel rol ekranı: biri gizli **imposter**, diğerleri **masum**. Masuma gizli **kalburüstü futbolcu adı**, imposter'a her oyun farklı eksenden **TEK bulanık ipucu** (pozisyon/dönem/kupa/fiziksel/büyük-milliyet — ifşa korumalı, en dar ipucu 155 oyuncuya uyar). **3 tur sıra-tabanlı kelime** (renkli oyuncu kartları, dikey kelime listesi; futbolcu/kulüp adı yazımı sunucuda yasak), sonra **gizli oylama** (kart seç/çekimser + joker sesi). İmposter NET en çok oyu almazsa (berabere/başkası elenirse) → **imposter kazanır** (Among Us). Gizli futbolcu havuzu `imposterPool.json` (kariyer-havuz.txt rank 1-585, %60/%40 bant). Sonuç: imposter + gizli futbolcu + ipucu + oy dökümü açılır. Rol/oy/cevap reveal'a kadar **sunucuda maskeli** (`viewImposterState`); imposter kimliği `crypto` rastgeleden (seed'den türetilemez); rol deadline'ı oyuncu ekrana girince başlar (found-gecikmesi yemesin). *v1: chat + bota-karşı yok (v2'ye).*
...
#### 🌐 Online Mod — gerçek zamanlı multiplayer (on mod)
...
**On modun da online'ı çalışıyor** — aynı mod-agnostik altyapının (eşleştirme, Ably, optimistic-lock, versiyon-GET, süre-dolumu) üzerine her mod yalnızca kendi sunucu motoru + client köprüsü + sayfa dalını ekledi (aşağıda "Diğer modlara yayma"). VS Düello/Hedefe/Kadro draft-veya-sıra-tabanlı; **Ortak Bul / Kariyer Yolu / 4'lü Kıyas** eşzamanlı (iki taraf aynı anda seçer); **Liste Doldur / Ortak Bul / Kariyer Yolu / 4'lü Kıyas / İmposter** cevap/rol sunucuda gizli (hile koruması); **Kareleri Kap/Zincir** kulüp-bazlı, puanlama sunucuda (bitişik grup BFS / keşişim) + öneri jokeri sunucu-otoriteli. **İmposter** ise 2-kişi deseninin ÖTESİNDE — N-kişi (3-5) lobi: yeni `match_player` tablosu + atomik N-claim + sıra-tabanlı kelime + gizli oylama (ilk N-kişi online modu, ileride takım/turnuvaya temel). Ayrıca **arkadaşını davet et** (özel maç linki — `matchmaking_queue.invite_code`, atomik claim) tüm modlarda.
...
- ✅ **DB şeması** — `match` (state + flowState + version + turnDeadline jsonb/kolonlar), `match_move` (audit/replay/reconnect, `(match_id, seq)` benzersiz), `matchmaking_queue` (+`invite_code` davet), **`match_player`** (çok-oyunculu/İmposter — `matchId`+`userId`+`playerIndex`, migration `0003`; 2-kişi modlar `p1/p2UserId` kullanmaya devam eder, dokunulmadı), `user_rating` (Elo şeması hazır, **hesap sonra** — herkes 1000'de başlar, UI'da gizli).
...
- ✅ **İmposter** (ÇOK-OYUNCULU 3-5, **N-kişi lobi** — 2-kişi deseninin ötesi) — [`lib/server/imposterMatchEngine.ts`](apps/web/src/lib/server/imposterMatchEngine.ts) (`ImposterMatchState`; sahne ROLE_REVEAL→WORDS→VOTE→RESULT; **`viewImposterState(side)` taraf-özel MASKELİ** — kim imposter/gizli futbolcu/oylar reveal'a kadar gizli; imposterIndex+gizli futbolcu **`crypto` rastgeleden**, public seed'den DEĞİL; yasak-kelime doğrulama sunucuda; süre dolunca pas) + [`api/match/[matchId]/imposter-move`](apps/web/src/app/api/match/[matchId]/imposter-move/route.ts) (side = `match_player` index, p1/p2 değil) + GET'te `getImposterMatch` (lazy-deadline: rol süresi oyuncu ekrana girince başlar) + [`useOnlineImposterMatch.ts`](apps/web/src/lib/useOnlineImposterMatch.ts) + [`imposter/[gameId]`](apps/web/src/app/imposter/[gameId]/page.tsx) (online-only). **Yeni `match_player` tablosu** (migration `0003`) + lobi matchmaking (`joinImposterLobby`/`tryFormImposterLobby` atomik N-claim, 30sn/5-dolu, zombi TTL) + `ONLINE_MODES`'a `'imposter'`. Mevcut 9 mod 2-kişi şemada hiç dokunulmadan çalışır.

> Tam plan, fazlar ve kararlar: [ONLINE-YOL-HARITASI.md](ONLINE-YOL-HARITASI.md) + [PLAN.md §19-23](PLAN.md). **On modun da online'ı tamam** (VS Düello · Hedefe · Kadro · Liste · Kareler · Zincir · Ortak Bul · Kariyer Yolu · 4'lü Kıyas · **İmposter** [3-5 kişi, yeni `match_player` + lobi]). Kalan: rating/Elo + İki Takım Ortak (veri hazır, ayrı mekanik) + İmposter v2 (chat + bota-karşı).
...
- ✅ **Online multiplayer** — **On modun da ONLINE'ı CANLI** (VS Düello · Hedefe Yaklaş · Kadro Kur · Liste Doldur · Kareleri Kap · Zincir Kur · Ortak Bul · Kariyer Yolu · 4'lü Kıyas · İmposter [3-5 kişi N-lobi]; sunucu-otoriteli + Ably hibrit push + arkadaş daveti; bkz. [🌐 Online Mod](#-online-mod--vs-düello-gerçek-zamanlı-multiplayer)).
...
│       │   │   ├── imposter/[gameId]/       İmposter route'u (3-5 kişi, online-özel)
│       │   │   ├── davet/[code]/            Arkadaş daveti (özel maç linki)
│       │   │   └── api/
│       │   │       ├── matchmaking/             Kuyruğa gir/çık + eşleşme (atomik 2-kişi + N-kişi lobi) + davet (invite_code)
│       │   │       └── match/[matchId]/         GET (versiyon-tabanlı) · move · {squad,target,list,squares,chain,common,career,quiz,imposter}-move · ably-token · transfer-options
│       │   ├── components/
...
│       │   │       ├── GameModeSelectScene.tsx   Ana oyun-modu seçimi (10 mod)
...
│       │   │       └── (İmposter sahneleri inline: imposter/[gameId]/page.tsx — Rol/Words/Vote/Result + renkli oyuncu kartı)
│       │   └── lib/
│       │       ├── server/               ONLINE sunucu katmanı:
│       │       │   ├── matchEngine.ts    VS Düello sunucu-otoriteli motor (doğrula/çöz/maskele)
│       │       │   ├── {target,squad,list,squares,chain,common,career,quiz,imposter}MatchEngine.ts  Mod-özel sunucu motorları
│       │       │   ├── matchmaking.ts    Atomik eşleştirme (DELETE...RETURNING) + N-kişi lobi + davet — ONLINE_MODES: 10 mod
│       │       │   └── ably.ts           Realtime publish + token üretimi
│       │       ├── useOnlineMatch.ts     ONLINE client köprüsü (Ably + poll + versiyon-GET + optimistic)
│       │       ├── useOnline{Target,Squad,List,Squares,Chain,Common,Career,Quiz,Imposter}Match.ts  Mod-özel client köprüleri
...
│       │       ├── imposterMode.ts       İmposter saf mantığı (gizli oyuncu + bulanık ipucu üreteci [ifşa korumalı] + yasak kelime + oylama çözümü + süre sabitleri)
│       │       ├── authErrors.ts         Better-Auth hata → güzel Türkçe çevirici (giriş/kayıt/sıfırlama)
...
│       └── public/
│           ├── data/                     players.json, clubs.json (build çıktısı) + clubPool/clubPairs/imposterPool.json (mod havuzları)
...
**10 mod canlı** (VS Düello · Kadro Kur · Hedefe Yaklaş · Liste Doldur · Kareleri Kap · Zincir Kur ·
Ortak Bul · Kariyer Yolu · 4'lü Kıyas · İmposter). İlk 9'u bota + arkadaşa + **online**; **İmposter** 3-5
kişilik **online-özel** sosyal dedüksiyon (yeni `match_player` tablosu + lobi matchmaking). Kalan: rating/Elo +
İki Takım Ortak (veri hazır, ayrı mekanik) + İmposter v2 (chat + bota-karşı). İleride bir "karma" mod düşünülebilir.
...
> Geriye kalan: **İki Takım Ortak** (veri hazır — `clubPairs.json` 1308 çift, ayrı mekanik). **İmposter ✅
> YAPILDI** (N-kişi lobi + `match_player`, [PLAN.md §23](PLAN.md)). Aşağısı bu modların **özgün tasarım
> analizi** (tarihsel karar günlüğü); güncel uygulama kararları [PLAN.md §14-23](PLAN.md)'te.
...
## 🕵️ "Futbol İmposter" — Sosyal Dedüksiyon Modu — ✅ YAPILDI (CANLI, v1)

> Among Us / kelime-çağrışım formatının ("Rayan Cherki") futbol + sosyal-dedüksiyon versiyonu.
> **Diğer tüm modlardan kategorik olarak farklı:** statik soru/cevap değil, **3-5 kişilik canlı
> çok-oyunculu** oyun → gerçek bir altyapı sıçraması (yeni `match_player` tablosu + lobi matchmaking).
> **Durum:** Tasarlandı, kodlandı, canlı (online-only v1). Tam uygulama: [PLAN.md §16, §23](PLAN.md).
> v1 kapsam-dışı (v2'ye): chat + bota-karşı. Aşağısı özgün tasarım analizi.
...
| Boyut | Durum |
|---|---|
| **✅ Altyapı** | **YAPILDI** — yeni `match_player` tablosu (5 kimlik) + lobi matchmaking (3-5 esnek, 30sn/5-dolu atomik N-claim) + sıra-tabanlı senkron + gizli oylama + `viewImposterState` rol gizliliği. Mevcut 9 mod 2-kişilik şemada hiç dokunulmadan çalışır. |
| **🕵️ İpucu** | **YAPILDI** — TEK kelime, her oyun farklı EKSEN: pozisyon · dönem · kupa · fiziksel · **ülke (yalnız 7 büyük milliyet — dar olanda başka eksene düşer = ifşa koruması)**. Kulüp ASLA. Ölçüldü: en dar ipucu 155 oyuncuya uyuyor (ifşa etmez). |
| **⌨️ Kelime** | Tek/iki-üç kelime, ~24 karakter. Yasak: futbolcu adı + kulüp adları (sunucu doğrular). |
| **🗳️ Oy vermeyen** | Çekimser serbest; kronik AFK süre-dolumuyla pas. İmposter NET en çok oyu almazsa (berabere dahil) → imposter kazanır. |
| **💬 Chat / 🤖 Bot** | v1 kapsam-dışı (v2'ye) — ilk sürüm chat'siz + online-only (bota-karşı yok). |

> Oyun tasarımı sağlam (Among Us iskeleti + futbol teması) + altyapı sıçraması tamamlandı. **CANLI (v1):**
> `app/imposter/[gameId]` (online-only) + `imposterMode.ts`/`imposterMatchEngine.ts` + `imposter-move` route +
> `useOnlineImposterMatch.ts`. Migration 0003 (match_player) Neon'a uygulandı. Tam günlük: [PLAN.md §23](PLAN.md).
