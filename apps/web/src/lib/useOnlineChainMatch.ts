...
/**
 * Online "Zincir Kur" maç köprüsü (client).
 *
 * `useOnlineSquaresMatch.ts`'in KARDEŞİ — aynı Ably + poll + versiyon-GET +
 * optimistic-retry iskeleti. `guess(playerId)` / `ackReveal`. Dönen `state` =
 * ChainMatchState (7 kulüp açık — maskeleme yok). `guess` outcome döner.
 */
export interface OnlineChainMatch {
  state: ChainMatchState | null;
  yourSide: 'P1' | 'P2' | null;
  status: string | null;
  loading: boolean;
  error: string | null;
...
export function useOnlineChainMatch(matchId: string | null): OnlineChainMatch {
  const [state, setState] = useState<ChainMatchState | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (nextStateJson !== lastStateJsonRef.current) {
        ...
        setState((data.state ?? null) as ChainMatchState | null);
      }
  ...
  const sendMove = useCallback(
    async (
      ...
      for (let attempt = 0; attempt <= MAX_RETRY; attempt++) {
        const res = await fetch(`/api/match/${matchId}/chain-move`, {
          method: 'POST',
  ...
  const guess = useCallback(
    async (playerId: string): Promise<ChainGuessOutcome | null> => {
      const data = await sendMove({ action: 'guess', playerId });
      return (data.outcome ?? null) as ChainGuessOutcome | null;
    },
    [sendMove],
  );
