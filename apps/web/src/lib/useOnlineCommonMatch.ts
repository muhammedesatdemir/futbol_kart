...
/**
 * Online "Ortak Bul" maç köprüsü (client).
 *
 * `useOnlineChainMatch.ts`'in KARDEŞİ — aynı Ably + poll + versiyon-GET +
 * optimistic-retry iskeleti. AMA akış EŞZAMANLI (snake değil): `select(playerId)`
 * bu turdaki tek seçim; `ackReveal` çift açılışını, `ackRound` tur sonucunu onaylar.
 * Dönen `state` = MASKELİ CommonMatchState (rakip seçimi + cevap havuzu gizli).
 */
export interface OnlineCommonMatch {
  state: CommonMatchState | null;
  yourSide: 'P1' | 'P2' | null;
  status: string | null;
  loading: boolean;
  error: string | null;
  /** Çift açılış ekranı görüldü → SELECT. */
  ackReveal: () => Promise<void>;
  /** Tur sonucu görüldü → sonraki tur / RESULT. */
  ackRound: () => Promise<void>;
  /** Bu turdaki ortak oyuncu seçimi — outcome döner (correct + playerId; puan gizli). */
  select: (playerId: string) => Promise<CommonSelectOutcome | null>;
  /** İpucu jokeri — ipucu içeriği döner (yalnız isteyene). 1×/maç. */
  useHint: () => Promise<CommonHintResult | null>;
...
export function useOnlineCommonMatch(matchId: string | null): OnlineCommonMatch {
  const [state, setState] = useState<CommonMatchState | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (nextStateJson !== lastStateJsonRef.current) {
        ...
        setState((data.state ?? null) as CommonMatchState | null);
      }
  ...
  const sendMove = useCallback(
    async (
      ...
      for (let attempt = 0; attempt <= MAX_RETRY; attempt++) {
        const res = await fetch(`/api/match/${matchId}/common-move`, {
          method: 'POST',
  ...
  const select = useCallback(
    async (playerId: string): Promise<CommonSelectOutcome | null> => {
      const data = await sendMove({ action: 'select', playerId });
      return (data.outcome ?? null) as CommonSelectOutcome | null;
    },
    [sendMove],
  );

  const useHint = useCallback(async (): Promise<CommonHintResult | null> => {
    const data = await sendMove({ action: 'use-hint' });
    return (data.hint ?? null) as CommonHintResult | null;
  }, [sendMove]);
